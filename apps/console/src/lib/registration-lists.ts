/**
 * Console → Registration-lists API client (M2.2, #33).
 *
 * Generic typed wrappers over `/console/registration-lists/<list>/*` — one set of verbs (list, create,
 ...
 * (409) comes back already localized; non-2xx throws a {@link RegistrationListApiError} carrying the
 * server's `messageKey`. DTO shapes mirror `apps/api`'s `registration-lists-route` (the console can't
 * import across the app boundary), keyed to the domain's `Locality`.
 */

import type { Locality } from "@vms/domain";
import { apiUrl } from "./api";

/** The minimum every registration-list row exposes; a list narrows this to its own shape below. */
export type MasterRow = { id: string; active: boolean };

export type BusinessEntityRow = MasterRow & {
  nameId: string;
  nameEn: string;
  category: Locality;
};
export type VendorCategoryRow = MasterRow & { nameId: string; nameEn: string };
export type CountryRow = MasterRow & { name: string; iso3: string };
export type CurrencyRow = MasterRow & {
  code: string;
  name: string;
  country: string | null;
  showInBankSelector: boolean;
};
export type BankRow = MasterRow & {
  name: string;
  code: string;
  location: Locality;
...
export class RegistrationListApiError extends Error {
  constructor(
...
const request = async <T>(
  ...
  const res = await fetch(
    apiUrl(`/console/registration-lists/${listPath}${path}${sep}lang=${locale}`),
    {
      credentials: "include",
  ...
  if (!res.ok) {
    ...
    const e = body.error ?? {};
    throw new RegistrationListApiError(
      res.status,
      e.code ?? "internal",
